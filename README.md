...
### 1. [PoC] Hard-Gated Topological Switching Engine
* **Documentation / 문서 링크**: [`README_v1_Topological.md`](./README_v1_Topological.md) (기존 `README.md`)
* **Core Source / 핵심 소스**: `egregore_system_test.py`
* **Description / 설명**:
* [EN] A Proof of Concept (PoC) stage that forcibly switches the weight space from `SPHERE` to `TORUS` when the threshold (0.85) is met based on cosine similarity. It is a prototype simulating cognitive psychological space.
* [KR] 코사인 유사도를 기준으로 문턱값(0.85) 충족 시 가중치 공간을 `SPHERE`에서 `TORUS`로 강제 스위칭하는 개념 증명(PoC) 단계입니다. 인지과학적 심리 공간 모사의 프로토타입입니다. 

### 2. [Production] Fully Differentiable, Energy-Conserved Architecture
* **Documentation / 문서 링크**: [`README_v2_Commercial.md`](./README_v2_Commercial.md) (기존 `README2.md`)
* **Core Source / 핵심 소스**: `commercial_alignment_system_test.py`
* **Description / 설명**:
* [EN] To overcome the gradient disconnection issue of hard switching, `Sigmoid Soft-Gating` was introduced. This production-ready version achieves perfect backpropagation and preserves the absolute weight magnitude (L2 Norm = 1) by establishing a hybrid control structure that confines the hypernetwork's modification variance within the normalization layer.
* [KR] 하드 스위칭의 미분 단절 문제를 극복하기 위해 `Sigmoid Soft-Gating`을 도입했습니다. 하이퍼네트워크의 변형 영향력을 정규화 안쪽에 가두는 하이브리드 제어 구조를 완성하여, 가중치 절대 크기(L2 Norm = 1)를 보존하고 완벽한 역전파(Backpropagation)를 달성한 상용 등급 버전입니다.
